    ...
    public class TestingUnitTests : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            LeanTween.delayedCall(gameObject, 0.2f, () =>
            {
                ...
                {
                    float
                        diffAmt = 1f; // This variable is dependent on a good frame-rate because it evalutes at the next Update
            ...
            LeanTween.alpha(cubeAlpha1, 0.5f, 0.1f)
                .setOnUpdate((float val) => { LeanTest.expect(val != 0f, "ON UPDATE VAL"); }).setOnCompleteParam("Hi!")
                .setOnComplete((object completeObj) =>
                {
        ...
        IEnumerator timeBasedTesting()
        {
            ...
            LeanTween.value(gameObject, new Vector3(1f, 1f, 1f), new Vector3(10f, 10f, 10f), 1f)
                .setOnUpdate((Vector3 val) => { didGetCorrectOnUpdate = val.x >= 1f && val.y >= 1f && val.z >= 1f; })
        ...
        IEnumerator lotsOfCancels()
        {
            ...
            for (int i = 0; i < aGOs.Length; i++)
            {
                ...
                tweensA[i] = LeanTween.move(cube, cube.transform.position + new Vector3(10f, 0, 0),
                    0.5f + 1f * (1.0f / (float) aGOs.Length)).id;
