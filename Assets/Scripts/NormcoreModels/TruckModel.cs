...
public partial class TruckModel : RealtimeModel
{
    public int owner
    {
        get { return _cache.LookForValueInCache(_owner, entry => entry.ownerSet, entry => entry.owner); }
        set
        {
    ...
    public UnityEngine.Vector3 explosionPoint
    {
        get
        {
            return _cache.LookForValueInCache(_explosionPoint, entry => entry.explosionPointSet,
                entry => entry.explosionPoint);
        }
        set
        {
            ...
            _cache.UpdateLocalCache(entry =>
            {
                entry.explosionPointSet = true;
                entry.explosionPoint = value;
                return entry;
            });
            InvalidateReliableLength();
    ...
    protected override int WriteLength(StreamContext context)
    {
        ...
        if (context.fullModel)
        {
            ...
            length += WriteStream.WriteBytesLength((uint) PropertyID.ExplosionPoint,
                WriteStream.Vector3ToBytesLength());
        }
        ...
        while (stream.ReadNextPropertyID(out uint propertyID))
        {
            switch (propertyID)
            {
                ...
                case (uint) PropertyID.ExplosionPoint:
                {
                    ...
                    bool explosionPointExistsInChangeCache =
                        _cache.ValueExistsInCache(entry => entry.explosionPointSet);
