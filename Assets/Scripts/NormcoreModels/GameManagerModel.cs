...
{
    [RealtimeProperty(1, true, true)] private int _currentSceneNumber;

    [RealtimeProperty(2, true, true)] private float _currentGameTimer;
...
public partial class GameManagerModel : RealtimeModel
{
    public int currentSceneNumber
    {
        get
        {
            return _cache.LookForValueInCache(_currentSceneNumber, entry => entry.currentSceneNumberSet,
                entry => entry.currentSceneNumber);
        }
        set
        {
            ...
            {
                entry.currentSceneNumberSet = true;
                entry.currentSceneNumber = value;
                return entry;
            });
    ...
    public double currentGameTimer
    {
        get
        {
            return _cache.LookForValueInCache(_currentGameTimer, entry => entry.currentGameTimerSet,
                entry => entry.currentGameTimer);
        }
        set
        {
            ...
            _cache.UpdateLocalCache(entry =>
            {
                entry.currentGameTimerSet = true;
                entry.currentGameTimer = value;
                return entry;
            });
            InvalidateReliableLength();
    ...
    protected override int WriteLength(StreamContext context)
    {
        ...
        if (context.fullModel)
        {
            ...
            length += WriteStream.WriteDoubleLength((uint) PropertyID.CurrentGameTimer);
        }
        else if (context.reliableChannel)
        {
            ...
            {
                length += WriteStream.WriteVarint32Length((uint) PropertyID.CurrentSceneNumber,
                    (uint) entry.currentSceneNumber);
            }
        ...
        while (stream.ReadNextPropertyID(out uint propertyID))
        {
            switch (propertyID)
            {
                case (uint) PropertyID.CurrentSceneNumber:
                {
                    ...
                    bool currentSceneNumberExistsInChangeCache =
                        _cache.ValueExistsInCache(entry => entry.currentSceneNumberSet);
                ...
                case (uint) PropertyID.CurrentGameTimer:
                {
                    ...
                    bool currentGameTimerExistsInChangeCache =
                        _cache.ValueExistsInCache(entry => entry.currentGameTimerSet);
    ...
    private void FlattenCache()
    {
        ...
        _currentGameTimer = (float) currentGameTimer;
        _cache.Clear();
    }
