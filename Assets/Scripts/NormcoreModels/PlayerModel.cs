...
public partial class PlayerModel : RealtimeModel
{
    ...
    {
        get { return _cache.LookForValueInCache(_forces, entry => entry.forcesSet, entry => entry.forces); }
        set
        {
            if (this.forces == value) return;
            _cache.UpdateLocalCache(entry =>
            {
                entry.forcesSet = true;
                entry.forces = value;
                return entry;
            });
            InvalidateReliableLength();
    ...
    protected override void Write(WriteStream stream, StreamContext context)
    {
        ...
        {
            LocalCacheEntry entry = _cache.localCache;
            if (entry.playerNameSet || entry.healthSet || entry.forcesSet || entry.idSet)
            {
