...
namespace Vectrosity
{
    public partial class VectorLine
    {
        ...
        {
            "VectorLine.SetColors: Length of color", "VectorLine.SetWidths: Length of line widths", "MakeCurve",
            "MakeSpline", "MakeEllipse"
        };
        ...
        bool CheckArrayLength(FunctionName functionName, int segments, int index)
        {
            ...
            if (index < 0)
            {
                Debug.LogError("VectorLine." + functionNames[(int) functionName] + ": The index value for \"" + name +
                               "\" must be >= 0");
                return false;
            }
            ...
            if (m_lineType == LineType.Points)
            {
                if (index + segments > pointsCount)
                {
                    if (index == 0)
                    {
                        Debug.LogError("VectorLine." + functionNames[(int) functionName] +
                                       ": The number of segments cannot exceed the number of points in the array for \"" +
                                       name + "\"");
                        return false;
                    }

                    Debug.LogError("VectorLine: Calling " + functionNames[(int) functionName] + " with an index of " +
                                   index + " would exceed the length of the Vector array for \"" + name + "\"");
                    return false;
                }
            ...
            if (m_lineType == LineType.Continuous)
            {
                if (index + (segments + 1) > pointsCount)
                {
                    ...
                    Debug.LogError("VectorLine: Calling " + functionNames[(int) functionName] + " with an index of " +
                                   index + " would exceed the length of the Vector array (" + pointsCount + ") for \"" +
                                   name + "\"");
                    return false;
                }
            }
        ...
        public void MakeRect(Vector3 bottomLeft, Vector3 topRight, int index)
        {
            if (m_lineType != LineType.Discrete)
            {
                if (index + 5 > pointsCount)
                {
                    ...
                    {
                        Debug.LogError(
                            "VectorLine.MakeRect: The length of the array for continuous lines needs to be at least 5 for \"" +
                            name + "\"");
                        return;
                    }

                    Debug.LogError("Calling VectorLine.MakeRect with an index of " + index +
                                   " would exceed the length of the Vector2 array for \"" + name + "\"");
                    return;
                }
            ...
            else
            {
                if (index + 8 > pointsCount)
                {
                    ...
                    {
                        Debug.LogError(
                            "VectorLine.MakeRect: The length of the array for discrete lines needs to be at least 8 for \"" +
                            name + "\"");
                        return;
                    }
        ...
        {
            MakeRoundedRect(new Vector2(rect.x, rect.y), new Vector2(rect.x + rect.width, rect.y + rect.height),
                cornerRadius, cornerSegments, 0);
        }

        public void MakeRoundedRect(Rect rect, float cornerRadius, int cornerSegments, int index)
        {
            MakeRoundedRect(new Vector2(rect.x, rect.y), new Vector2(rect.x + rect.width, rect.y + rect.height),
                cornerRadius, cornerSegments, index);
        }
            ...
            int index)
        {
            ...
            int neededCount = (m_lineType != LineType.Discrete)
                ? cornerSegments * 4 + 5 + index
                : cornerSegments * 8 + 8 + index;
            ...
            if (m_is2D)
            {
                ...
                CopyAndAddPoints(cornerPointCount, originalCount, 0,
                    new Vector2(topRight.x - bottomLeft.x, topRight.y - bottomLeft.y), index);
                if (m_lineType != LineType.Discrete)
                {
            ...
            else
            {
                ...
                CopyAndAddPoints(cornerPointCount, originalCount, 1,
                    new Vector2(topRight.x - bottomLeft.x, topRight.y - bottomLeft.y), index);
        ...
        private void CopyAndAddPoints(int cornerPointCount, int originalCount, int sectionNumber, Vector2 add,
            int index)
        {
            Vector3 add3 = add;
            for (int i = cornerPointCount - 1; i >= 0; i--)
            {
                if (m_lineType != LineType.Discrete)
                {
                    if (m_is2D)
                    {
                        m_points2[cornerPointCount * sectionNumber + i + index] =
                            m_points2[originalCount * sectionNumber + i + index] + add;
                    }
                    else
                    {
                        m_points3[cornerPointCount * sectionNumber + i + index] =
                            m_points3[originalCount * sectionNumber + i + index] + add3;
                    }
                }
                else
                {
                    if (m_is2D)
                    {
                        m_points2[cornerPointCount * sectionNumber + sectionNumber * 2 + i + index] =
                            m_points2[originalCount * sectionNumber + i + index] + add;
                    }
                    else
                    {
                        m_points3[cornerPointCount * sectionNumber + sectionNumber * 2 + i + index] =
                            m_points3[originalCount * sectionNumber + i + index] + add3;
                    }
                }
            }
        ...
        {
            MakeEllipse(origin, Vector3.forward, xRadius, yRadius, startDegrees, endDegrees, GetSegmentNumber(), 0.0f,
                0);
        }
        ...
        public void MakeArc(Vector3 origin, Vector3 upVector, float xRadius, float yRadius, float startDegrees,
            float endDegrees)
        {
        ...
        public void MakeArc(Vector3 origin, Vector3 upVector, float xRadius, float yRadius, float startDegrees,
            float endDegrees, int segments, int index)
        {
            MakeEllipse(origin, upVector, xRadius, yRadius, startDegrees, endDegrees, segments, 0.0f, index);
        }

        private void MakeEllipse(Vector3 origin, Vector3 upVector, float xRadius, float yRadius, float startDegrees,
            float endDegrees, int segments, float pointRotation, int index)
        {
            if (segments < 3)
            {
            ...
            {
                totalDegrees = (endDegrees > startDegrees)
                    ? endDegrees - startDegrees
                    : (360.0f - startDegrees) + endDegrees;
            ...
            if (m_lineType != LineType.Discrete)
            {
                ...
                if (m_is2D)
                {
                    ...
                    {
                        m_points2[index + i] =
                            v2Origin + new Vector2(.5f + Mathf.Sin(p) * xRadius, .5f + Mathf.Cos(p) * yRadius);
                        p += radians;
                    }
                ...
                {
                    var thisMatrix = Matrix4x4.TRS(Vector3.zero, Quaternion.LookRotation(-upVector, upVector),
                        Vector3.one);
                    for (i = 0; i < segments; i++)
                    {
                        m_points3[index + i] = origin +
                                               thisMatrix.MultiplyPoint3x4(new Vector3(Mathf.Sin(p) * xRadius,
                                                   Mathf.Cos(p) * yRadius, 0.0f));
                        p += radians;
                    }
            ...
            int index)
        {
            ...
            if (m_lineType != LineType.Discrete)
            {
                ...
                if (m_is2D)
                {
                    ...
                    for (int i = 0; i < end; i++)
                    {
                        m_points2[index + i] = GetBezierPoint(ref anchor1a, ref control1a, ref anchor2a, ref control2a,
                            (float) i / segments);
                    }
                }
                else
                {
                    for (int i = 0; i < end; i++)
                    {
                        m_points3[index + i] = GetBezierPoint3D(ref anchor1, ref control1, ref anchor2, ref control2,
                            (float) i / segments);
                    }
                }
            }
            ...
            else
            {
                ...
                if (m_is2D)
                {
                    ...
                    {
                        m_points2[index + idx++] = GetBezierPoint(ref anchor1a, ref control1a, ref anchor2a,
                            ref control2a, (float) i / segments);
                        m_points2[index + idx++] = GetBezierPoint(ref anchor1a, ref control1a, ref anchor2a,
                            ref control2a, (float) (i + 1) / segments);
                    }
                }
                else
                {
                    for (int i = 0; i < segments; i++)
                    {
                        m_points3[index + idx++] = GetBezierPoint3D(ref anchor1, ref control1, ref anchor2,
                            ref control2, (float) i / segments);
                        m_points3[index + idx++] = GetBezierPoint3D(ref anchor1, ref control1, ref anchor2,
                            ref control2, (float) (i + 1) / segments);
                    }
                }
            }
        }

        private static Vector2 GetBezierPoint(ref Vector2 anchor1, ref Vector2 control1, ref Vector2 anchor2,
            ref Vector2 control2, float t)
        {
        ...
        private static Vector3 GetBezierPoint3D(ref Vector3 anchor1, ref Vector3 control1, ref Vector3 anchor2,
            ref Vector3 control2, float t)
        {
            float cx = 3 * (control1.x - anchor1.x);
            float bx = 3 * (control2.x - control1.x) - cx;
            float ax = anchor2.x - anchor1.x - cx - bx;
        ...
        private void MakeSpline(Vector2[] splinePoints2, Vector3[] splinePoints3, int segments, int index, bool loop)
        {
            ...
            {
                Debug.LogError(
                    "VectorLine.MakeSpline was called with a Vector2 spline points array, but the line uses Vector3 points");
                return;
            }
            ...
            {
                Debug.LogError(
                    "VectorLine.MakeSpline was called with a Vector3 spline points array, but the line uses Vector2 points");
                return;
            }
            ...
            for (j = 0; j < numberOfPoints; j++)
            {
                ...
                if (m_lineType != LineType.Discrete)
                {
                    if (m_is2D)
                    {
                        ...
                        {
                            m_points2[pointCount++] = GetSplinePoint(ref splinePoints2[p0], ref splinePoints2[j],
                                ref splinePoints2[p2], ref splinePoints2[p3], i);
                        }
                    }
                    else
                    {
                        for (i = start; i <= 1.0f; i += add)
                        {
                            m_points3[pointCount++] = GetSplinePoint3D(ref splinePoints3[p0], ref splinePoints3[j],
                                ref splinePoints3[p2], ref splinePoints3[p3], i);
                        }
                    }
                }
                else
            ...
            if ((m_lineType != LineType.Discrete && pointCount < index + (segments + 1)) ||
                (m_lineType == LineType.Discrete && pointCount < index + (segments * 2)))
            {
                if (m_is2D)
                {
                    m_points2[pointCount] = GetSplinePoint(ref splinePoints2[p0], ref splinePoints2[j - 1],
                        ref splinePoints2[p2], ref splinePoints2[p3], 1.0f);
                }
                else
                {
                    m_points3[pointCount] = GetSplinePoint3D(ref splinePoints3[p0], ref splinePoints3[j - 1],
                        ref splinePoints3[p2], ref splinePoints3[p3], 1.0f);
                }
            }
        }
        ...
        private static void InitNonuniformCatmullRom(float x0, float x1, float x2, float x3, float dt0, float dt1,
            float dt2, ref Vector4 p)
        {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                else
                {
                    ...
                    if (m_is2D)
                    {
                        ...
                        {
                            m_points2[idx++] =
                                Vector2.Scale(VectorChar.data[charNum][j] + new Vector2(charPos, linePos),
                                    scaleVector) + (Vector2) startPos;
                        }
                    }
                    else
                    {
                        for (int j = 0; j < end; j++)
                        {
                            m_points3[idx++] =
                                Vector3.Scale(
                                    (Vector3) VectorChar.data[charNum][j] + new Vector3(charPos, linePos, 0.0f),
                                    scaleVector) + startPos;
                        }
                    }
        ...
        public void MakeWireframe(Mesh mesh)
        {
            ...
            if (m_is2D)
            {
                Debug.LogError("VectorLine.MakeWireframe can only be used with Vector3 points, which \"" + name +
                               "\" doesn't have");
                return;
            }
            ...
            for (int i = 0; i < mesh.subMeshCount; i++)
            {
                ...
                for (int j = 0; j < meshIndices.Length; j += polyCount)
                {
                    ...
                    {
                        CheckPairPoints(pairs, meshVertices[meshIndices[j + k]],
                            meshVertices[meshIndices[j + (k + 1) % polyCount]], linePoints);
                    }
                }
            }
        ...
        private static void CheckPairPoints(Dictionary<Vector3Pair, bool> pairs, Vector3 p1, Vector3 p2,
        ...
        public void MakeCube(Vector3 position, float xSize, float ySize, float zSize, int index)
        {
            ...
            if (m_is2D)
            {
                Debug.LogError("VectorLine.MakeCube can only be used with Vector3 points, which \"" + name +
                               "\" doesn't have");
                return;
            }
            ...
            if (index + 24 > pointsCount)
            {
                if (index == 0)
                {
                    Debug.LogError("VectorLine.MakeCube: The number of Vector3 points needs to be at least 24 for \"" +
                                   name + "\"");
                    return;
                }

                Debug.LogError("Calling VectorLine.MakeCube with an index of " + index +
                               " would exceed the length of the Vector3 points for \"" + name + "\"");
                return;
            }
