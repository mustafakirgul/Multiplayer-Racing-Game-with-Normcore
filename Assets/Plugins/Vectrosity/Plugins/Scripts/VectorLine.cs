...
namespace Vectrosity
{
    ...
    public partial class VectorLine
    {
        ...
        [SerializeField] Vector3[] m_lineVertices;
        ...
        [SerializeField] Vector2[] m_lineUVs;
        ...
        [SerializeField] Color32[] m_lineColors;
        ...
        [SerializeField] List<int> m_lineTriangles;
        ...
        [SerializeField] CanvasState m_canvasState;
        [SerializeField] bool m_is2D;
        ...
        [SerializeField] List<Vector2> m_points2;
        ...
        [SerializeField] List<Vector3> m_points3;
        ...
        int pointsCount
        {
            get { return m_is2D ? m_points2.Count : m_points3.Count; }
        }

        [SerializeField] Vector3[] m_screenPoints;
        [SerializeField] float[] m_lineWidths;
        ...
        [SerializeField] float m_maxWeldDistance;
        ...
        [SerializeField] float[] m_distances;
        ...
        [SerializeField] Texture m_originalTexture;
        ...
        [SerializeField] LineType m_lineType;
        ...
        public LineType lineType
        {
            ...
            set
            {
                if (value != m_lineType)
                {
                    ...
                    if (value != LineType.Continuous && ((m_points2 != null && m_points2.Count > 16383) ||
                                                         (m_points3 != null && m_points3.Count > 16383)))
                    {
                        Resize(16383);
                    }
        ...
        [SerializeField] float m_capLength;
        ...
        [SerializeField] bool m_smoothWidth = false;

        public bool smoothWidth
        {
            get { return m_smoothWidth; }
            set { m_smoothWidth = (m_lineType == LineType.Points) ? false : value; }
        }

        [SerializeField] bool m_smoothColor = false;
        ...
        [SerializeField] bool m_isAutoDrawing = false;
        ...
        [SerializeField] int m_drawStart = 0;
        ...
        [SerializeField] int m_drawEnd = 0;
        ...
        [SerializeField] int m_endPointsUpdate;
        ...
        [SerializeField] bool m_useNormals = false;
        [SerializeField] bool m_useTangents = false;
        [SerializeField] bool m_normalsCalculated = false;
        [SerializeField] bool m_tangentsCalculated = false;

        [SerializeField] EndCap m_capType = EndCap.None;
        [SerializeField] string m_endCap;
        ...
        [SerializeField] bool m_useCapColors = false;
        [SerializeField] Color32 m_frontColor;
        [SerializeField] Color32 m_backColor;
        [SerializeField] int m_frontEndCapIndex = -1;
        [SerializeField] int m_backEndCapIndex = -1;

        [SerializeField] float m_lineUVBottom;
        [SerializeField] float m_lineUVTop;
        [SerializeField] float m_frontCapUVBottom;
        [SerializeField] float m_frontCapUVTop;
        [SerializeField] float m_backCapUVBottom;
        [SerializeField] float m_backCapUVTop;
        [SerializeField] bool m_continuousTexture = false;
        ...
        [SerializeField] Transform m_drawTransform;
        ...
        [SerializeField] bool m_viewportDraw;
        ...
        [SerializeField] bool m_useTextureScale = false;
        ...
        [SerializeField] float m_textureOffset;
        ...
        [SerializeField] bool m_useMatrix = false;
        ...
        [SerializeField] bool m_collider = false;
        ...
        [SerializeField] bool m_trigger = false;
        ...
        [SerializeField] bool m_alignOddWidthToPixels = false;
        ...
        private void AddColliderIfNeeded()
        {
            if (m_go.GetComponent<Collider2D>() == null)
            {
                m_go.AddComponent((m_lineType == LineType.Continuous)
        ...
        protected void SetupLine(string lineName, Texture texture, float width, LineType lineType, Joins joins,
            bool use2D)
        {
            m_is2D = use2D;
            m_lineType = lineType;
        ...
        private void SetupTriangles(int startVert)
        {
            ...
            if (m_joins == Joins.Fill)
            {
                if (startVert >= 4)
                {
                    ...
                    m_lineTriangles[i] = startVert - 3;
                    m_lineTriangles[i + 1] = startVert;
                    m_lineTriangles[i + 2] = startVert + 3;
                    m_lineTriangles[i + 3] = startVert - 2;
                    m_lineTriangles[i + 4] = startVert;
                    m_lineTriangles[i + 5] = startVert + 3;
                }
                ...
                for (int i = startVert; i < end; i += 4)
                {
                    ...
                    m_lineTriangles.Add(i + 3); // Segment
                    m_lineTriangles.Add(i + 1);
        ...
        private void SetLastFillTriangles()
        {
            ...
            if ((m_is2D && m_points2[0] == m_points2[points2.Count - 1]) ||
                (!m_is2D && m_points3[0] == m_points3[points3.Count - 1]))
            {
                ...
                m_lineTriangles[i - 6] = m_vertexCount - 3;
                m_lineTriangles[i - 5] = 0;
                m_lineTriangles[i - 4] = 3;
                m_lineTriangles[i - 3] = m_vertexCount - 2;
                m_lineTriangles[i - 2] = 0;
                m_lineTriangles[i - 1] = 3;
            }
            else
            {
                if (m_lineTriangles[i - 4] == 3 && m_lineTriangles[i - 1] == 3)
                ...
                m_lineTriangles[i - 6] = 0;
                m_lineTriangles[i - 5] = 0;
                m_lineTriangles[i - 4] = 0;
                m_lineTriangles[i - 3] = 0;
                m_lineTriangles[i - 2] = 0;
                m_lineTriangles[i - 1] = 0;
            }
        ...
        private void SetupEndCap(float[] uvHeights)
        {
            ...
            {
                Debug.LogError("VectorLine: exceeded maximum vertex count of 65534 for \"" + m_name +
                               "\"...use fewer points");
                return;
            }
            ...
            if (m_joins == Joins.Fill)
            {
                ...
                {
                    m_lineTriangles.Insert(idx, i);
                    m_lineTriangles.Insert(1 + idx, i + 1);
                    m_lineTriangles.Insert(2 + idx, i + 3);
                    m_lineTriangles.Insert(3 + idx, i + 1);
                    m_lineTriangles.Insert(4 + idx, i + 2);
                    m_lineTriangles.Insert(5 + idx, i + 3);
                    idx += 6;
                }
            }
            else
        ...
        private void Resize()
        {
            ...
            if (m_lineType != LineType.Points)
            {
                originalSegmentCount = (m_lineType == LineType.Continuous)
                    ? Mathf.Max(0, m_pointsCount - 1)
                    : m_pointsCount / 2;
            }
        ...
        private bool SetVertexCount()
        {
            ...
            {
                Debug.LogError("VectorLine: exceeded maximum vertex count of 65534 for \"" + name +
                               "\"...use fewer points (maximum is 16383 points for continuous lines and points, and 32767 points for discrete lines, minus two if end caps are used)");
                return false;
            }
        ...
        public static void SetCamera3D()
        {
            ...
            {
                Debug.LogError(
                    "VectorLine.SetCamera3D: no camera tagged \"Main Camera\" found. Please call SetCamera3D with a specific camera instead.");
                return;
            }
        ...
        public void SetColors(List<Color32> lineColors)
        {
            ...
            {
                Debug.LogError("VectorLine.SetColors: Length of lineColors list in \"" + name +
                               "\" must be same length as points list");
                return;
            }
        ...
        private void SetWidths(List<float> lineWidthsFloat, List<int> lineWidthsInt, int arrayLength, bool doFloat)
        {
            ...
            if (m_lineType == LineType.Points)
            {
                ...
                {
                    Debug.LogError(
                        "VectorLine.SetWidths: line widths list must be the same length as the points list for \"" +
                        name + "\"");
                    return;
                }
            }
        ...
        public static VectorLine SetLine(Color color, float time, params Vector3[] points)
        {
            ...
            var line = new VectorLine("SetLine", new List<Vector3>(points), null, 1.0f, LineType.Continuous,
                Joins.None);
            line.color = color;
            if (time > 0.0f)
            {
        ...
        public static VectorLine SetLine3D(Color color, float time, params Vector3[] points)
        {
            ...
            var line = new VectorLine("SetLine3D", new List<Vector3>(points), null, 1.0f, LineType.Continuous,
                Joins.None);
            line.color = color;
        ...
        {
            var line = new VectorLine("SetRay",
                new List<Vector3>(new Vector3[] {origin, new Ray(origin, direction).GetPoint(direction.magnitude)}),
        ...
        {
            var line = new VectorLine("SetRay3D",
                new List<Vector3>(new Vector3[] {origin, new Ray(origin, direction).GetPoint(direction.magnitude)}),
                null, 1.0f, LineType.Continuous, Joins.None);
            line.color = color;
            line.Draw3DAuto(time);
        ...
        private void DrawEndCap(bool draw3D)
        {
            ...
            if (m_capType <= EndCap.Mirror)
            {
                ...
                if (!draw3D)
                {
                    var d = (m_lineVertices[vIndex] - m_lineVertices[vIndex + 1]).normalized *
                            m_lineWidths[widthIndex] * 2.0f * capDictionary[m_endCap].ratio1;
                    var d2 = d * capDictionary[m_endCap].offset1;

                    m_lineVertices[m_vertexCount] =
                        m_lineVertices[vIndex] + d + d2; // Set end cap vertex positions, including offset
                ...
                {
                    ScaleCapVertices(m_vertexCount, capDictionary[m_endCap].scale1,
                        (m_lineVertices[m_vertexCount + 1] + m_lineVertices[m_vertexCount + 2]) / 2);
                }
            ...
            if (m_capType >= EndCap.Both)
            {
                ...
                if (!draw3D)
                {
                    var d = (m_lineVertices[vIndex - 2] - m_lineVertices[vIndex - 1]).normalized *
                ...
                {
                    ScaleCapVertices(m_vertexCount + 4, capDictionary[m_endCap].scale2,
                        (m_lineVertices[m_vertexCount + 4] + m_lineVertices[m_vertexCount + 7]) / 2);
                }
        ...
        private void Line2D(int start, int end, Matrix4x4 thisMatrix, bool useTransformMatrix)
        {
            ...
            for (int i = start; i < end; i += add)
            {
                ...
                if (m_capLength == 0.0f)
                {
                    ...
                    m_lineVertices[idx].x = p1.x - px.x;
                    m_lineVertices[idx].y = p1.y - px.y;
                    m_lineVertices[idx + 3].x = p1.x + px.x;
                    m_lineVertices[idx + 3].y = p1.y + px.y;
                ...
                m_lineVertices[idx + 2].x = p2.x + px.x;
                m_lineVertices[idx + 2].y = p2.y + px.y;
                m_lineVertices[idx + 1].x = p2.x - px.x;
                m_lineVertices[idx + 1].y = p2.y - px.y;
                idx += 4;
            ...
            if (m_joins == Joins.Weld)
            {
                if (m_lineType == LineType.Continuous)
                {
                    WeldJoins(start * 4 + (start == 0 ? 4 : 0), end * 4,
                        Approximately(m_points2[0], m_points2[m_pointsCount - 1]));
                }
                else
                {
                    if ((end & 1) == 0)
        ...
        private void Line3D(int start, int end, Matrix4x4 thisMatrix, bool useTransformMatrix)
        {
            ...
            var cameraPlane = new Plane(camTransform.forward,
                camTransform.position + camTransform.forward * cam3D.nearClipPlane);
            ...
            for (int i = start; i < end; i += add)
            {
                ...
                if ((pos1.x == pos2.x && pos1.y == pos2.y) || IntersectAndDoSkip(ref pos1, ref pos2, ref p1, ref p2,
                    ref screenHeight, ref ray, ref cameraPlane))
                {
                    SkipQuad(ref idx, ref widthIdx, ref widthIdxAdd);
                    continue;
                }
                ...
                if (m_capLength == 0.0f)
                {
                    ...
                    m_lineVertices[idx].x = pos1.x - px.x;
                    m_lineVertices[idx].y = pos1.y - px.y;
                    m_lineVertices[idx + 3].x = pos1.x + px.x;
                    m_lineVertices[idx + 3].y = pos1.y + px.y;
                    if (doSmooth && i < end - add)
                    {
                        px.x = pos2.y - pos1.y;
                        px.y = pos1.x - pos2.x;
                ...
                m_lineVertices[idx + 2].x = pos2.x + px.x;
                m_lineVertices[idx + 2].y = pos2.y + px.y;
                m_lineVertices[idx + 1].x = pos2.x - px.x;
                m_lineVertices[idx + 1].y = pos2.y - px.y;
                idx += 4;
                widthIdx += widthIdxAdd;
            }
            ...
            if (m_joins == Joins.Weld && end - start > 1)
            {
                ...
                {
                    WeldJoins(start * 4 + (start == 0 ? 4 : 0), end * 4,
                        start == 0 && end == m_pointsCount - 1 &&
                        Approximately(m_points3[0], m_points3[m_pointsCount - 1]));
                }
                else
                {
                    if ((end & 1) == 0)
        ...
        public void Draw3D()
        {
            ...
            if (m_is2D)
            {
                Debug.LogError("VectorLine.Draw3D can only be used with a Vector3 array, which \"" + name +
                               "\" doesn't have");
                return;
            }
            ...
            if (m_joins == Joins.Weld && end - start > 1)
            {
                ...
                if (m_lineType == LineType.Continuous)
                {
                    WeldJoins3D(start * 4 + (start == 0 ? 4 : 0), end * 4,
                        start == 0 && end == m_pointsCount - 1 &&
                        Approximately(m_points3[0], m_points3[m_pointsCount - 1]));
                }
                else
                {
                    ...
                    WeldJoinsDiscrete3D(start + 1, end,
                        start == 0 && end == m_pointsCount - 1 &&
                        Approximately(m_points3[0], m_points3[m_pointsCount - 1]));
                }
            }
        ...
        private bool IntersectAndDoSkip(ref Vector3 pos1, ref Vector3 pos2, ref Vector3 p1, ref Vector3 p2,
            ref float screenHeight, ref Ray ray, ref Plane cameraPlane)
        {
        ...
        private void DrawPoints()
        {
            ...
            if (m_is2D)
            {
                for (int i = start; i <= end; i++)
                {
                    ...
                    m_lineVertices[idx].x = p1.x + v2.x;
                    m_lineVertices[idx].y = p1.y + v2.y;
                    m_lineVertices[idx + 3].x = p1.x - v1.x;
                    m_lineVertices[idx + 3].y = p1.y - v1.y;
                    m_lineVertices[idx + 1].x = p1.x + v1.x;
                    m_lineVertices[idx + 1].y = p1.y + v1.y;
                    m_lineVertices[idx + 2].x = p1.x - v2.x;
                    m_lineVertices[idx + 2].y = p1.y - v2.y;
                    idx += 4;
                }
            }
            else
            {
                ...
                {
                    p1 = useMatrix
                        ? cam3D.WorldToScreenPoint(thisMatrix.MultiplyPoint3x4(m_points3[i]))
        ...
        {
            var l1a = m_lineVertices[p1];
            var l1b = m_lineVertices[p2];
            var l2a = m_lineVertices[p3];
            var l2b = m_lineVertices[p4];
        ...
        private void SetTextureScale()
        {
            ...
            if (m_is2D)
            {
                for (int i = 0; i < end; i += add)
                {
                    ...
                    else
                    {
                        if (useTransformMatrix)
                        {
                            p1 = thisMatrix.MultiplyPoint3x4(new Vector2(m_points2[i].x * Screen.width,
                                m_points2[i].y * Screen.height));
                            p2 = thisMatrix.MultiplyPoint3x4(new Vector2(m_points2[i + 1].x * Screen.width,
                                m_points2[i + 1].y * Screen.height));
                        }
                        else
                        {
                    ...
                    px.x = p2.x - p1.x;
                    px.y = p2.y - p1.y;
                    float xPos = thisScale / (m_lineWidths[widthIdx] * 2 /
                                              ((float) System.Math.Sqrt((px.x * px.x) + (px.y * px.y)) + capAdd));
                    m_lineUVs[idx].x = offset;
                    m_lineUVs[idx + 3].x = offset;
            ...
            else
            {
                ...
                for (int i = 0; i < end; i += add)
                {
                    ...
                    px.x = p1.x - p2.x;
                    px.y = p1.y - p2.y;
                    float xPos = thisScale /
                                 (m_lineWidths[widthIdx] * 2 / (float) System.Math.Sqrt(px.x * px.x + px.y * px.y));
                    m_lineUVs[idx].x = offset;
                    m_lineUVs[idx + 3].x = offset;
                    m_lineUVs[idx + 2].x = xPos + offset;
                    m_lineUVs[idx + 1].x = xPos + offset;
        ...
        private void SetPathWorldVerticesContinuous(ref int i, ref Vector3 v3, ref int startIdx, ref int endIdx,
            Vector2[] path)
        {
        ...
        public static List<Vector3> BytesToVector3List(byte[] lineBytes)
        {
            ...
            {
                Debug.LogError(
                    "VectorLine.BytesToVector3Array: Incorrect input byte length...must be a multiple of 12");
                return null;
            }
        ...
        public void SetDistances()
        {
            ...
            if (m_distances == null || m_distances.Length !=
                ((m_lineType != LineType.Discrete) ? pointsCount : pointsCount / 2 + 1))
            {
            ...
            if (is2D)
            {
                if (m_lineType != LineType.Discrete)
                {
                    for (int i = 0; i < thisPointsLength; i++)
                    {
                        ...
                        totalDistance +=
                            System.Math.Sqrt(diff.x * diff.x +
                                             diff.y * diff
                                                 .y); // Same as Vector2.Distance, but with double instead of float
            ...
            else
            {
                if (m_lineType != LineType.Discrete)
                {
                    for (int i = 0; i < thisPointsLength; i++)
                    {
                        ...
                        totalDistance +=
                            System.Math.Sqrt(diff.x * diff.x + diff.y * diff.y +
                                             diff.z * diff
                                                 .z); // Same as Vector3.Distance, but with double instead of float
                        m_distances[i + 1] = (float) totalDistance;
                    }
                }
                else
        ...
        public Vector2 GetPoint(float distance, out int index)
        {
            ...
            if (m_lineType != LineType.Discrete)
            {
                point = Vector2.Lerp(m_points2[index - 1], m_points2[index],
                    Mathf.InverseLerp(m_distances[index - 1], m_distances[index], distance));
            }
            else
            {
                point = Vector2.Lerp(m_points2[(index - 1) * 2], m_points2[(index - 1) * 2 + 1],
                    Mathf.InverseLerp(m_distances[index - 1], m_distances[index], distance));
            }
        ...
        public Vector3 GetPoint3D(float distance, out int index)
        {
            ...
            if (m_lineType != LineType.Discrete)
            {
                point = Vector3.Lerp(m_points3[index - 1], m_points3[index],
                    Mathf.InverseLerp(m_distances[index - 1], m_distances[index], distance));
            }
            else
            {
                point = Vector3.Lerp(m_points3[(index - 1) * 2], m_points3[(index - 1) * 2 + 1],
                    Mathf.InverseLerp(m_distances[index - 1], m_distances[index], distance));
            }
        ...
        public static void SetEndCap(string name, EndCap capType, float offsetFront, float offsetBack,
            params Texture2D[] textures)
        {
            SetEndCap(name, capType, offsetFront, offsetBack, 1.0f, 1.0f, textures);
        }

        public static void SetEndCap(string name, EndCap capType, float offsetFront, float offsetBack, float scaleFront,
            float scaleBack, params Texture2D[] textures)
        {
            ...
            {
                Debug.LogError("VectorLine.SetEndCap (\"" + name +
                               "\"): must supply two textures when using SetEndCap with EndCap.Front, EndCap.Back, or EndCap.Mirror");
                return;
            }
            ...
            if (capType == EndCap.Both)
            {
                ...
                {
                    Debug.LogError("VectorLine.SetEndCap (\"" + name +
                                   "\"): must supply three textures when using SetEndCap with EndCap.Both");
                    return;
                }
            ...
            else if (capType == EndCap.Back)
            {
                backPixels =
                    GetRotatedPixels(
                        frontTex); // Since there's only one texture, it's the front, even if it's used for the back
            ...
            capDictionary.Add(name,
                new CapInfo(capType, tex, ratio1, ratio2, offsetFront, offsetBack, scaleFront, scaleBack, uvHeights));
        }
        ...
        public bool Selected(Vector2 p, int extraDistance, int extraLength, out int index, Camera cam)
        {
            if (cam == null)
            {
                ...
                {
                    Debug.LogError(
                        "VectorLine.Selected: camera cannot be null. If there is no camera tagged \"MainCamera\", supply one manually");
                    index = 0;
                    return false;
                }
            ...
            if (m_lineType == LineType.Points)
            {
                ...
                if (m_is2D)
                {
                    for (int i = m_drawStart; i <= end; i++)
                    {
                        ...
                        thisPoint = useTransformMatrix
                            ? (Vector2) thisMatrix.MultiplyPoint3x4(m_points2[i])
                            : m_points2[i];
                        if (m_viewportDraw)
                        {
                        ...
                        if (p.x >= thisPoint.x - size && p.x <= thisPoint.x + size && p.y >= thisPoint.y - size &&
                            p.y <= thisPoint.y + size)
                        {
                            index = i;
                            return true;
                        }
                ...
                for (int i = m_drawStart; i <= end; i++)
                {
                    ...
                    float size = m_lineWidths[wIdx] + extraDistance;
                    thisPoint = useTransformMatrix
                        ? cam.WorldToScreenPoint(thisMatrix.MultiplyPoint3x4(m_points3[i]))
                        : cam.WorldToScreenPoint(m_points3[i]);
                    if (p.x >= thisPoint.x - size && p.x <= thisPoint.x + size && p.y >= thisPoint.y - size &&
                        p.y <= thisPoint.y + size)
                    {
                        index = i;
            ...
            if (m_is2D)
            {
                for (int i = m_drawStart; i < end; i += add)
                {
                    ...
                    if ((p - (p1 + t * (p2 - p1))).sqrMagnitude <=
                        (m_lineWidths[wIdx] + extraDistance) * (m_lineWidths[wIdx] + extraDistance))
                    {
