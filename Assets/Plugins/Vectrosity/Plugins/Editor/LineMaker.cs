...
public class LineMaker : ScriptableWizard
{
    ...
    static void CreateWizard()
    {
        ...
        if (!go)
        {
            EditorUtility.DisplayDialog("No object selected", "Please select an object in the scene hierarchy",
                "Cancel");
            return;
        }
    ...
    void OnFocus()
    {
        ...
        {
            selectedSegmentsString =
                "Selected line segment " + ((selectedSegments.Count > 1) ? "indices: " : "index: ");
    ...
    Vector3 Vector3Round(Vector3 p)
    {
        ...
        return new Vector3((float) System.Math.Round(p.x, 3), (float) System.Math.Round(p.y, 3),
            (float) System.Math.Round(p.z, 3));
    }

    private void WriteLineToFile()
    {
        var path = EditorUtility.SaveFilePanelInProject("Save " + (useVector2 ? "Vector2" : "Vector3") + " Line",
            objectTransform.name + "Vector.bytes", "bytes",
    ...
    private void SetLineScale()
    {
        ...
        foreach (GameObject line in lines)
        {
            line.transform.localScale = new Vector3(lineScale * objectScale.x, lineScale * objectScale.y,
    ...
    private void LoadLineFile()
    {
        ...
        {
            EditorUtility.DisplayDialog("Incorrect file length",
                "File does not seem to be a " + (canUseVector2 ? "2" : "3") + "D line file", "Cancel");
            return;
        }
        ...
        {
            vectorArray[count++] = new Vector3(ConvertBytesToFloat(bytes, i), ConvertBytesToFloat(bytes, i + 4),
                (canUseVector2 ? 0 : ConvertBytesToFloat(bytes, i + 8)));
        }
