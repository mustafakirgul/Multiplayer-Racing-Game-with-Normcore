...
public class VectorObject2DEditor : Editor
{
    ...
    class Point
    {
        // Since Vector2 can't directly modify x or y
        public float x;
        public float y;
    ...
    class Point3
    {
        // Since Vector3 can't directly modify x, y, or z
        public float x;
        public float y;
        public float z;
    ...
    private void ShowTexture()
    {
        ...
        {
            EditorGUILayout.HelpBox("Texture must have the Clamp Mode set to Repeat in order for texture scale to work",
                MessageType.Warning);
        }
    ...
    private void ShowPoints3D()
    {
        ...
        for (int i = 0; i < points3.Count; i++)
        {
            ...
            if (vline.points3[i].x != points3[i].x || vline.points3[i].y != points3[i].y ||
                vline.points3[i].z != points3[i].z)
            {
    ...
    {
        return (points.Count < 16383 && vline.lineType != LineType.Discrete) ||
               (points.Count < 32765 && vline.lineType == LineType.Discrete);
    }

    private bool CanAddPoint3D()
    {
        return (points3.Count < 16383 && vline.lineType != LineType.Discrete) ||
               (points3.Count < 32765 && vline.lineType == LineType.Discrete);
    }
    ...
    private void OnSceneGUI()
    {
        ...
        Handles.BeginGUI();
        GUILayout.BeginArea(
            new Rect((Screen.width / EditorGUIUtility.pixelsPerPoint) - 180,
                (Screen.height / EditorGUIUtility.pixelsPerPoint) - 150, 170, 102), "", "box");
        ...
        else
        {
            ...
            GUILayout.Space(
                60); // The line point gizmos can't be selected for some reason if the actual slider is visible
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();
...
#if OLDUNITY
			Vector2 sliderPoint =
 Handles.Slider2D (AdjustedPoint (vline.points2[i]), v3forward, v3right, v3up, scenePointSize, Handles.SphereCap, 1.0f);
#else
            Vector2 sliderPoint = Handles.Slider2D(AdjustedPoint(vline.points2[i]), v3forward, v3right, v3up,
                scenePointSize, Handles.SphereHandleCap, 1.0f);
#endif
    ...
    private void AddScenePoint(Vector2 mousePos)
    {
        ...
        int idx = (Vector2.Distance(adjustedMousePos, vline.points2[vline.points2.Count - 1]) < point0Distance)
            ? vline.points2.Count - 1
            : 0;
        if (lineType == LineType.Discrete)
        {
            // Adding points to the beginning of a discrete line isn't user-friendly
...
#else
        Handles.SphereHandleCap(0, AdjustedPoint(vline.points2[idx]), Quaternion.identity, scenePointSize,
            EventType.Repaint);
#endif
        if (distance > 40.0f)
        {
...
#else
            Handles.SphereHandleCap(0, AdjustedPoint(vline.points2[idx2]), Quaternion.identity, scenePointSize,
                EventType.Repaint);
#endif
    ...
    private void MoveScenePoints(Vector2 mousePos)
    {
        ...
        if (lineType == LineType.Continuous)
        {
            ...
            {
                Handles.DrawDottedLine(AdjustedPoint(vline.points2[i]) + offset,
                    AdjustedPoint(vline.points2[i + 1]) + offset, 4);
            }
        }
...
#else
                Handles.DotHandleCap(0, AdjustedPoint(vline.points2[i]) + offset, Quaternion.identity, lineWidth,
                    EventType.Repaint);
#endif
            }
        }
    ...
    private void UpdateLine()
    {
        ...
        if (!Application.isPlaying)
        {
            ...
            UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty(UnityEditor.SceneManagement.EditorSceneManager
                .GetActiveScene());
        }
    }
}